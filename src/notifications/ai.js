...
export async function generateEmbedding(aiBinding, text) {
    ...
    if (_aiCallCount >= MAX_AI_CALLS_PER_INVOCATION) {
        logger.warn(`[AI] Subrequest budget exhausted (${MAX_AI_CALLS_PER_INVOCATION} calls used). Skipping embedding.`);
        return [];
    }
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            lastErr = err;
            const isTransient = err.message?.includes('temporarily unavailable') ||
                                err.message?.includes('503') ||
                                err.message?.includes('9000');

            if (attempt < MAX_RETRIES && isTransient) {
                const delay = RETRY_DELAYS_MS[attempt];
                logger.warn(`[AI] Transient embedding error (attempt ${attempt + 1}/${MAX_RETRIES + 1}), retrying in ${delay}ms: ${err.message}`);
                await sleep(delay);
            } else {
                // Non-transient error or retries exhausted — log and give up
                if (isTransient) {
                    logger.warn(`[AI] Embedding failed after ${MAX_RETRIES + 1} attempts: ${err.message}`);
                } else {
